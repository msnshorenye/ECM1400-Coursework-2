...
def index():
    """
    Makes the template using render template by intialising a
    game board and then using it as board with the html template 
    This function is called when the template 
    and the flask framework web server is loaded. 
    """
    g_b  = initialise_board() #make the board
    return render_template('Othello Board.html',game_board = g_b) #makes the template 
#when the web server is opened up
@app.route('/move', methods = ['GET', 'POST'])

def get_move():
   """
   Docstring for get_move
   Gets the coordinates the player clicked and
   then plays the players turn and Ai players
   turn as well if the game is not over
   Returns a success or fail json data for
   the template
   If the game is over it returns 'finished' data

   """
   if request.method == 'GET': #if the request method is equal 
      global Current_Player #making current player global
      #so that It can be used in the get move function
      global move_counter #making move counter global so
      #that it can be decreased in the get move function
      print('This is the game board')
      print_board(g_b_2)
      x = request.args.get('x', type=int) #getting the x coordinate 
      #that has been clicked by the player
      print(x)
      y = request.args.get('y', type=int) #getting the y coordinate 
      #that has been clicked by the player
      print(y)
      coord_list = [x-1, y-1] #zero indexing means the coordinates
      logging.info(F"Dark coordinates played {x} and {y}")
      any_moves = check_if_any_possible_move(g_b_2, Current_Player) #checks if their are any possible 
      #moves to be made by the current player
      ...
      if move_counter > 0 and any_moves is True and board_full is True: #if all the game conditions
         ...
         any_moves = check_if_any_possible_move(g_b_2, Light) #function call to
         #make sure the ai can play a move on the board
         if any_moves is True: #if there is an available move
            ...
            if any_moves is True and ai_move is False: #If the ai cant make a move
               #but their is an available move 
               #their should be a turn played
               return{'status': 'success', 'player' : switch_colour(Current_Player), 'board': g_b_2}
            else:
          ...
          return{'finished': check_win(g_b_2), 'board': g_b_2}
        #if their are no more moves allowed or avaliable or the board is full
         #the game is finished so it returns
         #the board and winner to the html template
...
def save_board():
   ...
   if request.method == 'GET':
      return{'board': g_b_2} #returns the game board 
   #so that it can be saved in a json Stringfy data type
