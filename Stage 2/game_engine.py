...
def main():
    logging.basicConfig(filename="file.log", level= logging.DEBUG, format = "%(asctime)s %(levelname)s %(message)s", datefmt = "")
    logging.basicConfig(filename="info.log", level= logging.INFO, format = "%(asctime)s %(levelname)s %(message)s", datefmt = "")
...
def cli_coords_input():
    ...
    except ValueError: #if an integer isnt entered the error should be caught
        logger.exception("Wrong data type")


    coord_list = [x,y] #take both inputs and puts them into a list 
    return coord_list #returns the coord_list so that it can be used by legal_turbn
...
def check_if_any_possible_move(board,colour, size =8):
    ...
    for x in range(0,size): # we have to check every space on the board 
        for y in range(0,size):
            ...
            if board[x][y] == "None   ":
                if x+1 <= 7 and board[x+1][y] == opposite_colour: #same logic for each direction possible in each move
                    y_distance = 0
                    x_distance = 1
                    a = x+1
                    b = y
...
def ai_player_move(board, colour = "Light",  size = 8):
        ...
        coord_list = [] #list for the tuple coordinates that will get flanked
        opposite_colour = "Dark   "
        best_move = "" #assigning variables so that they can be changed later on
        for x in range(0,size): #every position on the board is checked with a nested loop
            for y in range(0,size): 
                 all_possible_flips_list = [] #This list stores every possible 
                 to_flip =[]
                 if colour == "Light": #same logic to select the Two diffrent colours as the game board has space padding on Dark and Light
                   opposite_colour = "Dark   "
                   actual_colour = "Light  "
                 elif colour == "Dark":
                    opposite_colour = "Light  "
                 ...
                 if board[x][y] == "None   ": #The space has to be empty to play a move on it
                    if x+1 <= 7 and board[x+1][y] == opposite_colour : #same logic of legal turn for each direction for each coordinate
                        go = True #boolean which is true while the loop of checking counters to be flipped
                        to_flip_1 = [] #make a list for each individual direction 
                        y_distance = 0
                        x_distance = 1
                        a = x+1
                        b = y
                        to_flip_1.append((a, b)) #add the counter of opposite colour to the flipped list
                        to_flip_1.append((x, y)) #adding the coordinate being played to the list to flip this done 
                        while 0 <=a<=7 and 0<=b<=7 and board[a][b]==opposite_colour and go is True: #while the coordinates are still opposite colour and Light hasnt been found
                            a = a + x_distance
                            b = b + y_distance #increase the coordinate in the direction that the initial opposite colour was found so we can look for a valid move to take
                            to_flip_1.append((a, b)) #if it is an opposite counter to be flipped 
                            if 0 <= a  <= 7 and  0<= b <= 7: #checking whether the next coordinates are legally in the board
                                if board[a][b] == actual_colour: #checks whether their is one of their own counters in the same direction making it a valid move
                                        go = False #boolean changed as the loop should end as no more counters need to be flipped as it is one of their own
                                        legal_turn = True #We now know that their is a legal move so we can set legal turn to true
                                        all_possible_flips_list.append(to_flip_1) #add the list of counters flipped to the right to list of all directions flipped
        ...
        if legal_turn is True: #If their is legal move on the board
            ...
            for (i, j) in coord_list: #iterates through the coordinate list which is the longest flips in a direction 
...
def check_board_full(the_board, size = 8):
    ...
    for x in range(0, size): #iterate through every position on the board with a nested loop
        for y in range(0, size):
            if the_board[x][y] == "None   ": #If there is an empty space return true if not return false
                return True
    return False
...
def simple_game_loop():
    ...
    move_counter = 60 #60 total moves as the game starts with 60 available spaces on the board
    current_colour = "Dark" 
    opposite_colour = "Light"
    ...
    while move_counter > 0 and available_move is True: #the players should keep playing while the 
        available_move1 = check_if_any_possible_move(board2, current_colour) #check if the current colour has any moves that 
        available_move2 = check_if_any_possible_move(board2,opposite_colour) #check if the other player has any moves available
        if available_move1 and available_move2 is False: #If both are false then the game loop should be over
            print('No available moves')
            break
        if check_if_any_possible_move(board2, current_colour) is True: #If there are available moves for dark they can play
            ...
            players_move_1 = legal_move(current_colour, current_coords, board2) #checks if legal move then plays it for the coordinates dark player gave
            print('\n')
            ...
            available_move = check_if_any_possible_move(board2, opposite_colour) #checks if the opposite colour has any moves available.
            if check_if_any_possible_move(board2, opposite_colour) is True: #if it is true then Light can select their move
                print('Lights turn')
...
def check_win(game_board, size = 8):
    ...
    """
    light_counter = 0 #both light and dark counters are set to zero before they are counted
    dark_counter = 0
    for row  in range(0,size):
        for column in range(0,size): #iterate through 
            if game_board[row][column] == "Light  ": #if the coordinate or 2d index is equal to light 
                ...
                light_counter += 1
            elif game_board[row][column] == "Dark   ": #if a coordinate is equal to Dark then the dark counter should be increased 
                dark_counter += 1
    if light_counter > dark_counter: #if more light counters are counted on the board then it return a string announcing light as the winner
        light_string = 'Light won the game with ', light_counter,' disks on the board'
        return light_string
    if dark_counter > light_counter: #if dark has more counters then a string should be returned announcing dark as the winner.
