...
def main():
    logging.basicConfig(filename="components.log", level= logging.DEBUG, format = "%(asctime)s %(levelname)s %(message)s", datefmt = "")
...
def legal_move(colour, coordinate, board, size = 8): 
    ...
    :param coordinate: Set of coordinates obtain from the player 
    from input statements or cli_coordnates_input 
    that are used to check whether the turn is legal
    :param board: The gameboard which is a 2d array that represents the current game board  state in Ascii
    ...
    """
    legal_turn = False #Legal turn is set to false 
    #intially as a legal move has not been found yet
    ...
    if board[x][y] == "None   ": # For a move to be made the position has to be none 
        #otherwise it already has an assigned counter

        if x+1 <= 7 and board[x+1][y] == opposite_colour: #all adjacent directions coordinates 
        #within the size of the board are checked with selection
